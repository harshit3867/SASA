...
// ================= LIVE PROJECTION FETCH (HYDERABAD: J → M) =================
  ...
  fetch(PROJECTION_CSV_URL)
    ...
    .then(csvText => {
      ...
      reportData = rows.map(row => {
        // Column A → Date
        const dateStr = row[Object.keys(row)[0]];
        const [dd, mm, yyyy] = dateStr.split("/");
        const dateObj = new Date(`${yyyy}-${mm}-${dd}`);

        // Column J → M ONLY (last 4 stores)
        const storeColumns = Object.keys(row).slice(9, 13);

        const stores = {};
        let total = 0;

        storeColumns.forEach(colName => {
          const normalized = colName.trim().replace(" Mnow", "_mnow");
          if (!HYDERABAD_STORES.includes(normalized)) return;
    ...
    .catch(err => {
      console.error("Hyderabad projection fetch failed", err);
      alert("Failed to load Hyderabad projections");
    });
}
...
<table class="table table-bordered final-merged-table"><thead><tr>
        <th>Store Name</th>
...
const allStores = HYDERABAD_STORES.filter(
  s => projectionData[s] || summaryData[s]
);
...
mergedHTML += `<tfoot><tr style="background:#ff9800;color:#000;font-weight:bold;">
        <td>Total</td>`;
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        ...
        <button id="downloadImageBtn"
            style="background:#ff9800;color:#000;border:none;
    ...
    <div id="finalTableWrapper">
        ${mergedHTML}
    </div>
`;
document.getElementById("downloadImageBtn").addEventListener("click", function () {
    const target = document.getElementById("finalTableWrapper");

    if (!target) {
        alert("Final table not found");
        return;
    }
    ...
    html2canvas(target, {
        ...
        link.download = `Final_Merged_Table_${new Date().toISOString().slice(0,10)}.png`;
    ...
    function updateMergedFooterTotals() {
        ...
        for (let i = 0; i < 3; ++i) {
            ...
            if (footerCell) {
                if (i === 2) { // BF column: show average across Hyderabad stores
                    ...
                    let avg = bfSum / HYDERABAD_STORES.length;
...
const finalStores = HYDERABAD_STORES;
