...
function processFile() {
    ...
    reader.onload = function (e) {
        ...
        fileData.forEach(order => {
            ...
            if (!ALLOWED_STORES.includes(storeName)) return;
...
<table class="table table-bordered final-merged-table">
  <thead class="orange-header">
    ...
    const allStores = ALLOWED_STORES.filter(
  store => projectionData[store] || summaryData[store]
);
    ...
    mergedHTML += `<tfoot><tr style="background:#ff9800;color:#fff;font-weight:bold;">
        <td>Total</td>`;
...
document.getElementById("downloadImageBtn").addEventListener("click", function () {
    ...
    html2canvas(target, {
        ...
        link.download = `Final_Merged_Table_Bangalore_${new Date().toISOString().slice(0,10)}.png`;
...
// ================= LIVE PROJECTION FETCH (ONLY B → I) =================
  ...
  fetch(PROJECTION_CSV_URL)
    ...
    .then(csvText => {
      ...
      reportData = rows.map(row => {
        ...
        // Column B → I ONLY (first 8 stores)
        const storeColumns = Object.keys(row).slice(1, 9);

        const stores = {};
        let total = 0;

        storeColumns.forEach(storeName => {
          const value = Number(row[storeName]) || 0;
          stores[storeName.trim()] = value;
          total += value;
        });
      ...
      // 🔥 Use existing logic (NO CHANGE)
      generateSummaryTable();
    })
    .catch(err => {
      console.error("Live projection fetch failed", err);
      alert("Unable to load live projections");
    });
}
